...
This lecture was delivered remotely with the professor recovering from illness. The main objectives were to finish the transformer architecture discussion, introduce the concept of positional embeddings, and touch on Mixture of Experts (MoE). The professor also outlined the remaining course topics: Vision Transformers (ViTs), symbolic reasoning, neurosymbolic reasoning (to reduce hallucinations), planning without interactions, and reinforcement learning (planning with interactions).

**Midterm Context**: The professor briefly addressed midterm results, noting the median was around 60-65 with a maximum of 87. He reassured students that letter grade thresholds would be adjusted since the midterm accounts for 20% of the total grade.

---

## Part 1: Review of Previous Material

### Simple RNN Architecture

The course had previously covered:

- Simple RNN layers and how they differ from dense layers
- The need for vector hidden states to represent latent variables required for modeling time series structure across multiple examples
- The decoupling of hidden states from predictions (using dimensionality reduction followed by dot products for scalar outputs)

### Backpropagation Through Time (BPTT)

When you unroll an RNN architecture across time steps, backward propagation becomes "backpropagation through time." The critical factor is the weight matrix W' (W prime) used in the recurrent connection. Based on the eigendecomposition of this matrix:

- Gradients can **vanish** (diminish) if eigenvalues are less than 1
- Gradients can **explode** if eigenvalues are greater than 1

This was compared to infinite impulse response (IIR) filters in signal processing.

### LSTM Architecture

Long Short-Term Memory networks address gradient flow problems by borrowing ideas from **highway networks**—creating direct pathways from input to output. Key features include:

- **Forget gate**: Equipped with sigmoidal outputs (vectors where each element is between 0 and 1)
- **Input gate** and **output gate**: Control what information propagates forward
- **Long-term memory (S(t))**: Separate from the short-term hidden state
- **Vector-in, vector-out sigmoidal units**: Allow selective propagation of different dimensions

While LSTMs are largely replaced by transformers for NLP, the concept of recurrency combined with state-space methods led to architectures like **Mamba**, which competes with transformers.

### Language Modeling with RNNs

Language modeling predicts the next token given a context of previous tokens. In the RNN setup:

- Previous N tokens enter at the input
- They pass through embedding layers
- Form long-term or short-term hidden states
- Propagate forward to predict the next token

### Neural Machine Translation

This task uses an **encoder-decoder architecture** (sequence-to-sequence model):

- The **encoder** (in orange) produces a "thought vector" φ by looking bidirectionally at hidden states
- This single thought vector generates all translations—a limitation improved by transformer encoder-decoder architectures
- **Teacher forcing**: During training, ground truth is fed at each decoder step; during inference, the previous predicted token is fed instead
- **Bidirectionality**: Looking at context in both directions helps in many NLP tasks

### Beam Search and MLSE

The **Maximum Likelihood Sequence Estimation (MLSE)**, also known as the Viterbi Decoder, keeps track of multiple decoding trajectories to calculate the likelihood of entire sequences rather than using a greedy, memoryless approach at each step.

- **Beam search** is the practical implementation of this concept
- The "beam width" (number of branches) is a hyperparameter
- This approach is optimal compared to greedy single-stage decoding

The **BLEU metric** was mentioned as an earlier NLP evaluation metric with precision semantics, though many other metrics exist today.
